...
larger GPUs. Today the largest GPU's in the market contain ~50 SM's (NVIDIA's Pascal containing
56); across a benchmark set of 41 applications (later described in Section~\ref{sec:methodology})
...
than today's biggest GPUs.  \hl{For those that do not, we find that the absolute number
of CTA's generated is intentionally limited by the programmer or the problem dataset can not
be easily scaled up (e.g. scaling the dataset requires new algorithmic changes).
While most of these applications will are unlikely to scale to thousands of GPUs across an 
entire data center due, Figure~\ref{fig:ctas} shows that programmer transparent workload scaling (up to 8$\times$) will
be attractive to many GPU developers for applications that already contain sufficient algorithmic and data parallelism.}
