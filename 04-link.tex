...
\hl{In order to avoid instability we set a threshold between 
sample intervals corresponding to 5\% of the link bandwidth, below which we do 
not perform reconfiguration, more advanced control techniques could leverage hysteresis to prevent unnecesary oscilations}.
...
around in \hl{just tens of cycles or less} already is in use by modern high bandwidth 
...
can be sufficiently projected ahead of time as assumed in this work by using fixed, periodic
link turn points.}
