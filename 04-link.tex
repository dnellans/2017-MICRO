...
with symmetric static bandwidth assignment on 8 inter-socket communication links.
Each sub-link is comprised of multiple equal number uni-directional high-speed
...
bandwidth scenarios. On the other hand, multi-socket link bandwidth utilization can have
...
scenario is all CTAs writing to the same memory range at the end of a
...
Such communication patterns typically utilizes only
...
lanes, similarly to NVLink sub-links~\cite{pascal-tesla-wp}. In these links, 8 
...
always re-trained to contain symmetric link bandwidth with 4 lanes per 
...
link load balancer re-trains and reverses the direction of one of the 
...
configuration, links are then retrained back towards a symmetric configuration 
...
respect to our SW locality optimize GPU by exploring different values of the
...
One additional caveat worth noting is that for asymmetric interconnect links optimize
variance among individual links while total switch bandwidth remains constant.
