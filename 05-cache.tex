...
At the beginning, we allocate one half of the cache ways for local, and another half for caching 
remote data (Step \circled{0}). After the sampling period, we collect the 
bandwidth values on the outgoing NVLink and local memory channels. Link 
utilization above 99\% is considered to be saturated (Step \circled{1}). 
...
(Step \circled{2}). If it is a write-back cache (only L2), dirty lines from the 
...
remote, and allocates it now to a set of local ways (Step \circled{3}). That way, our NUMA-aware cache partitioning algorithm reduces 
