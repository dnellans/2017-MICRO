...
Similarly, multi-socket, and multi-node homogeneous and heterogeneous CPU
...
IFINITY~\cite{dgx,INTELQPI,AMDINFINITYFABRIC}. These modern fabrics utilize serial
high speed signalling technologies on uni-directional lanes collectively
comprizing full-duplex links. This way each link capacity is statically
allocated at design time and usually is symmetric in nature. 
On the contrary, in this paper we propose to
dynamically re-allocate available link bandwidth resources by using same on-board
and on-system wires and on-chip I/O interfaces, while implementing both recever
and transmitter drivers at each lane. This approach resembles previously proposed tri-state
bi-directional bus technologies ~\cite{tri-state}, or former Intel front-side
